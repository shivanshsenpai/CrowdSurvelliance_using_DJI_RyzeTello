...
def choose(x):
    ...
    if x==0:
        ...
        choice='fire'
    print(f"Button clicked! Current choice: {choice}")

def nothing(x):
    pass

# Function to draw people count on the frame
def draw_frame(frame,total_count=None, count=None,density_alert=False,weapon_alert=False,fire_alert=False):
    # cv2.createTrackbar('choice','choose',0,2,nothing)

    y=40
    cv2.putText(frame, f"Altitude: {altitude}", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255, 255), 2)
    y+=40
    if count:
        cv2.putText(frame, f"current People: {count}", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
        y+=40
    # if total_count:
    #     cv2.putText(frame, f"Total People: {total_count}", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
    #     y+=40


    if density_alert:
        cv2.putText(frame, "Alert: High Density!", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0,255), 2)
        y+=40

    if weapon_alert:
        cv2.putText(frame, "Alert: Weapon Detected!", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0,255), 2)
        y+=40
    
    if fire_alert:
        cv2.putText(frame, "Alert: Fire Detected!", (20, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0,255), 2)
        y+=40
...
# Main function
def main():
    global altitude,choice
    drone = Tello()
    drone.connect()
    drone.streamon()
    cv2.namedWindow('Detection Output')
    ...
    try:
        while True:
            ...
            height_change=drone.get_height()

            if height_change<0 and altitude+height_change>=0:
                altitude+=height_change

            elif height_change>0 and altitude+height_change<=250:
                altitude+=height_change

            if choice=='human':
                # Detect humans and count them cumulatively
                frame,total_human_count, human_count,density_alert = detect_humans(frame)
                if density_alert:
                    densityAlertThread=threading.Thread(target=beep)
                    densityAlertThread.start()
                draw_frame(frame,total_human_count, human_count,density_alert)
            elif choice=='weapon':
                # Detect weapons
                ...
                draw_frame(frame,None,None,None,weapon_alert)
            elif choice=='fire':
                # Detect fire
                frame,fire_alert = detect_fire(frame)
                if fire_alert:
                    fireAlertThread=threading.Thread(target=beep)
                    fireAlertThread.start()
                draw_frame(frame,None,None,None,None,fire_alert)
